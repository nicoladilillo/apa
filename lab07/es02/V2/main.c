...
int promising(int *collana, int pos, int val, int *finale, int *cnt, int *cnt_val,
        int rpt, int max_rpt, int n_z, int n_s, Item *occ, int next, int before)
{
    /*
    ...
    */
    switch (before) {
        case zaffiri:
            if (next != zaffiri && next != rubini)
                return 0;
            break;
        case smeraldi:
            if (next != smeraldi && next != topazi)
                return 0;
            break;
        case rubini:
            if (next != smeraldi && next != topazi)
    ...
    */
    if (val + occ[next].v> *cnt_val && n_z <= n_s)
        swap(cnt, cnt_val, finale, pos, val + occ[next].v, collana);
...
int powerset(Item *occ, int pos, int val, int *collana, int max, int *cnt, int max_val,
        int *cnt_val, int rpt, int max_rpt, int *f, int n_z, int n_s, int c)
{
    int i;
    ...
    for (i = 0; i < pietre; i++) {
         /*
         * inseriamo una pietra solo se sono ancora disponibile una delle sue occorrenze e se
         * aggiungendolo in lista rispettiamo tutte le nostro condizioni stabilite dalla promising
         */
        if (occ[i].n > 0) {
            ...
            /* chiamata ricorsiva */
            if (promising(collana, pos, val, f, cnt, cnt_val, rpt, max_rpt, n_z, n_s, occ, i, c) &&
                powerset(occ, pos + 1, val + occ[i].v, collana, max, cnt, max_val, cnt_val, tmp, max_rpt, f, n_z, n_s, i))
                return 1;
...
void wrapper_powerset(Item *v, int max, int max_val, FILE *f, int rpt)
{
    ...
    for (i = 0; i < pietre; i++){
        ...
        nz = (i == zaffiri) ? 1 : 0;
        ns = (i == smeraldi) ? 1 : 0;
        powerset(v, 1, v[i].v, collana, max, &cnt, max_val, &cnt_val, 1, rpt, finale, nz, ns, i);
        v[i].n++;
    }
